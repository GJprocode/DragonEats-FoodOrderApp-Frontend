...
const MobileNavLinks = () => {
    ...
    const checkAdminRole = async () => {
      try {
        if (user?.email) {
          ...
          const response = await axios.get(`${API_BASE_URL}/api/admin/check-admin/${user.email}`, {
            headers: {
  ...
  return (
    <>
      <Link to="/manage-restaurant" className="flex bg-white items-center font-bold hover:text-green-500">
        Manage Restaurant
      </Link>
      ...
      {isAdmin && (
        <Link to="/admin-panel" className="flex bg-white items-center font-bold hover:text-green-500">
          Admin
        </Link>
      )}
